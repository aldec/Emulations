...
-M microblaze-fdt   -serial mon:stdio -serial /dev/null -display none \
...
-machine-path ./$project_name/qemu_cosim/tmp -device loader,addr=0xfd1a0074,data=0x1011003,data-len=4 -device loader,addr=0xfd1a007C,data=0x1010f03,data-len=4 &
...
-gdb tcp::9000   -net nic -net nic -net nic -net nic,netdev=eth0 -netdev user,id=eth0,tftp=/tftpboot,hostfwd=tcp::1534-:1534,hostfwd=tcp::5555-:22    \
-hw-dtb ./src/qemu/zcu102-arm.cosim.multi.dtb \
-icount 1 -sync-quantum $SYNC_QUANTUM \
-machine-path ./$project_name/qemu_cosim/tmp -global xlnx,zynqmp-boot.cpu-num=0 -global xlnx,zynqmp-boot.use-pmufw=true   -m 4G
